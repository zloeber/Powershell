<#
.SYNOPSIS
Create a new organization share on svpr-file-a2 and setup the DFS share and all permission groups and settings.
.DESCRIPTION
Create a new department share on svpr-file-a2 and setup the DFS share and all permission groups and settings.
.PARAMETER DeptName
...
.EXAMPLE
New-OrgShare -DeptName "BusDevSales"
...
param(
    ...
    [string]$DeptName,
    [Parameter(Position = 1, HelpMessage="Enter a share name")]
    [string]$ShareName = $null,
    [Parameter(Position = 2, HelpMessage="Domain Name (FQDN)")]
    [string]$Domain = 'Contoso.com',
    [Parameter(Position = 3, HelpMessage="Domain Name (NetBIOS)")]
    [string]$ShortDomain = 'CONTOSO',
    [Parameter(Position = 4, HelpMessage = 'File path where the share will be created')]
    [string]$FilePath = 'D:\OrgShares\',
    [Parameter(Position = 5, HelpMessage = 'DFSN root server location')]
    [string]$DFSNServer = 'Fileserver.contoso.com',
    [Parameter(Position = 6, HelpMessage = 'Location where the groups will be created')]
...
$sharepath = "$($FilePath)$($ShareName)"

# AD Groups
$ReadOnlyGroup = "perm_OrgShare-$($sharename)_ReadOnly"
$FullAccessGroup = "perm_OrgShare-$($sharename)_FullAccess"
$DenyGroup = "perm_OrgShare-$($sharename)_DenyAccess"
...
$dfsfoldername = "\\$($Domain)\OrgShare\$($sharename)"
$FullAccessGroups = @('Administrators',$FullAccessGroup)
$dfscmd1 = ('dfsutil property sd grant \\$($Domain)\OrgShare\<0> ' + $ShortDomain + '\<1>:F protect') -replace '<0>',$sharename -replace '<1>',$FullAccessGroup
$dfscmd2 = ('dfsutil property sd grant \\$($Domain)\OrgShare\<0> ' + $ShortDomain + '\<1>:RX protect') -replace '<0>',$sharename -replace '<1>',$ReadOnlyGroup
...
try {
    ...
    New-ADGroup -Name $ReadOnlyGroup -SAMAccountName $ReadOnlyGroup -Description "Dept Share $ShareName - Read Only Access" -GroupCategory Security -Path "OU=Permission,OU=Service Groups,DC=CONTOSO,DC=org" -GroupScope Global
}
...
try {
    ...
    New-ADGroup -Name $DenyGroup -SAMAccountName $DenyGroup -Description "Dept Share $ShareName - Deny Access" -GroupCategory Security -Path "OU=Permission,OU=Service Groups,DC=CONTOSO,DC=org" -GroupScope Global
}
...
try {
    ...
    New-ADGroup -Name $FullAccessGroup -SAMAccountName $FullAccessGroup -Description "Dept Share $ShareName - Full Access" -GroupCategory Security -Path "OU=Permission,OU=Service Groups,DC=CONTOSO,DC=org" -GroupScope Global
}
...
if ($smbshares -notcontains $share.NewShareName) {
    ...
    if (-not (Test-Path $sharepath)) {
        ...
        $objUser_RO = New-Object System.Security.Principal.NTAccount("CONTOSO\$($ReadOnlyGroup)")
        $objUser_Full = New-Object System.Security.Principal.NTAccount("CONTOSO\$($FullAccessGroup)")
        $objUser_Deny = New-Object System.Security.Principal.NTAccount("CONTOSO\$($DenyGroup)")
    ...
    if (Test-Path $sharepath) {
        ...
        if ($dfsnfolder -eq $null) {
           # $dfsnPath = $share.FutureShare -replace '\\\\',''
            ...
            Write-Host ""
            New-DfsnFolder -Path $dfsfoldername -TargetPath "\\$($DFSNServer)\$($sharename)"
